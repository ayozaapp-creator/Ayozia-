...
export default function FollowersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await authFetch(`${API}/users/${id}/followers`);
      const json = await res.json();
      const list: PublicUser[] = Array.isArray(json.followers)
        ? json.followers
        : [];
      setData(list);
      setFiltered(list);
    } catch (e) {
      console.warn("followers load error:", e);
      setData([]);
      setFiltered([]);
    } finally {
      setLoading(false);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>Noch keine Follower</Text>
        }
